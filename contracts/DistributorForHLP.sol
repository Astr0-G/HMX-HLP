...
contract DistributorForGLP is IDistributor {
    ...
    constructor(address hlpaddress) public {
        gov = msg.sender;
        admin = msg.sender;
        hlp = hlpaddress;
    }
    ...
    function _mintGLP(uint256 amount) public payable {
        HLP(hlp)._mint(msg.sender, amount);
    }
    ...
    function _updateLastDistributionTime(address _receiver) private {
        lastDistributionTime[_receiver] = block.timestamp.div(DISTRIBUTION_INTERVAL).mul(
